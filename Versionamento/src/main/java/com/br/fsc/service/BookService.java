...
public class BookService {
	...
	/*
	 * public BookDtoV2 createV2(BookDtoV2 bookDtoV2){ //cancelamos o id de book
	 * bookDtoV2.setId(null); //criamos um mapeador ModelMapper mapper = new
	 * ModelMapper(); //convertemos o bookDto em um entity //Book book =
	 * mapper.addConverter(Converter<BookDto.class, Book.class> bookDtoV2.);
	 * //bookDtoV2.setTitle(book.getTitle()), //
	 * bookDtoV2.setDescription(book.getDescription()),
	 * //bookDtoV2.setPages(book.getPages()), //
	 * bookDtoV2.setValue(book.getValue()), // bookDtoV2.setDate(new Date()));
	 * //salvamos os valores book =bookRepository.save(book); //copiamos o book para
	 * bookDto bookDtoV2.setId(book.getId()); //retornamos o bookDto return
	 * bookDtoV2; }
	 */
	public List<BookDto> readAll() {
		var book = bookRepository.findAll();

		var Dto = book.stream().map(bookDto -> new ModelMapper().map(bookDto, BookDto.class))
				.collect(Collectors.toList());

		Dto.stream().forEach(p -> p.add(linkTo(methodOn(BookController.class).obterPorId(p.getId())).withSelfRel()));
		return Dto;
	}
	...
	public Optional<BookDto> readId(Long id) {
		...
		// bookDto.add((((Link)
		// methodOn(BookRepository.class).findAll()).withSelfRel()));
